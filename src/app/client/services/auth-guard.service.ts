...
export class AuthGuard implements CanActivate {
  ...
  canActivate(route,state:RouterStateSnapshot) { 
	...
	// }
	if(state.url=='/home' ||state.url=='/welcome_register'){   // new header component show
	...
	if(state.url=='/home' ){   // new header component show		
	  ...
	  if(this.authService.isLoggedIn()==true){
		  ...
		  //alert(state.url);
		   if(state.url=='/login' || state.url.split('/')[1]=='resetpass'||(state.url.split('/')[1]=='register' && state.url.split('/')[2]==undefined)||state.url=='/invitation'||state.url=='/forgotpassword'||(state.url.split('/')[1]=='contactus' && state.url.split('/')[2]==undefined) || state.url=='/pagenotfound'|| state.url=='/home'){
