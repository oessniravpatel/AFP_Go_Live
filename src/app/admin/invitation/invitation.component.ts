...
export class InvitationComponent implements OnInit {
	...
	default(){
		...
		this.InvitationEntity.UserId ='';
		this.InvitationEntity.IndustryId ='';
		this.company_sel = false;
		this.InvitationService.getAllCompany()
			...
			this.InvitationService.getrole(this.globals.authData.RoleId)
			//.map(res => res.json())
			.then((data) => {
	...
	addInvitation(InvitationForm) {
		...
		if((this.companyhide==false && this.InvitationEntity.CompanyId>0) || (this.companyhide==true)){

			this.company_sel = false;
		if (InvitationForm.valid) {
			this.btn_disable = true;
			this.globals.isLoading = true;
			// if(this.InvitationEntity.RoleId==3){
			// 	this.InvitationEntity.UserId = '84';
			// } else {
