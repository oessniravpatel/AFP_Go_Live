	...
	public function add_AssessmentDetails($post_AssessmentDetails) {
		...
		if($post_AssessmentDetails) {
			...
			if($res) {				
				...
				$res = array();
				foreach($obj as $row) {
					$data = $this->db->query('SELECT '.$insert_id.' as CAssessmentId,ksa.KSAId as KSAId FROM tblmstksa AS ksa  WHERE ksa.IsActive=1 and ksa.CAreaId = '.$row->CAreaId.' order by RAND() LIMIT '.$row->destksa);	
