...
class KSA_model extends CI_Model
 {
	...
	public function get_ksadata($ksa_id = NULL) {
		...
		if($ksa_id) {
			...
			$this->db->select('ksa.KSAId,ksa.Name,ksa.CAreaId,ksa.IsActive,(SELECT COUNT(ck.CKSAId) FROM tblcandidateksa as ck Left join tblcandidateassessment as cass on ck.CAssessmentId=cass.CAssessmentId WHERE ck.KSAId=ksa.KSAId and cass.EndTime is NULL) as isdisabled');
