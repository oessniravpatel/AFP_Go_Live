...
class Settings_model extends CI_Model
 {
	...
	public function add_teamsize($post_teamsize) {
		$this->db->select('DisplayOrder,TeamSizeId');
		$this->db->where('DisplayOrder',$post_teamsize['DisplayOrder']);
		$result = $this->db->get('tblmstteamsize');	
		if($result->num_rows()==0)
		{
		if($post_teamsize) {
			if(isset($post_teamsize['DisplayOrder']) && !empty($post_teamsize['DisplayOrder'])){
				$DisplayOrder = $post_teamsize['DisplayOrder'];
			}	else {
				$DisplayOrder = '';
			}
	...
	public function getlist_teamsize() {
		...
		$this->db->select('DisplayOrder,TeamSizeId,TeamSize,IsActive');
